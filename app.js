...
async function validarLogin() {
    ...
    try {
        const response = await fetch(`${URL_LOGIN}?action=login&user=${encodeURIComponent(user)}&pass=${encodeURIComponent(pass)}`);
        ...
        alert("Error de conexión con el servidor de usuarios.");
    } finally {
...
function actualizarVista() {
    ...
    proyecto.forEach((p, idx) => {
        ul.innerHTML += `<li>
            <span><b>${p.cantidad}x</b> ${p.nombre}</span>
            <i class="fas fa-trash-alt" style="color:#e74c3c; cursor:pointer" onclick="eliminarItem(${idx})"></i>
        </li>`;
...
async function imprimirPDF() {
    ...
    doc.text("REPORTE DE MATERIALES", 105, 20, {align:"center"});
